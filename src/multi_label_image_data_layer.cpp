...
void MultiLabelImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  label_num_ = this->layer_param_.multi_label_image_data_param().label_num();
  ...
  while (infile >> filename) {
    label.clear();
    // read label_num labels, contains au label and landmark label
    for(int il = 0; il < label_num_; il++){
      int ilbl;
      infile>>ilbl;
...
void MultiLabelImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for(int ilbl = 0; ilbl < label_num_; ilbl ++){
        prefetch_label[item_id*label_num_ + ilbl] = lines_[lines_id_].second[ilbl];
    }
